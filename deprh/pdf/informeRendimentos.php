...
$bordas = 0;
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(0, $altura, 'R$ ' . printValor($infoRendimentos['informeRendimento']['RIAP'] + $infoRendimentos['informeRendimento']['RIO']), $bordas, 1, 'R');
$ordinateFinal = $pdf->GetY();
$pdf->Line(($netWidth - $larguraValores + $margins['left']), $ordinate_2, ($netWidth + $margins['left']), $ordinate_2);
$pdf->SetY($ordinate_2);
$linha['x2'] = $netWidth + $margins['left'];
...
$pdf->SetX(0);
$pdf->SetLeftMargin($margins['left']);
$pdf->MultiCell(0, ($altura - 2), utf8_decode('Os rendimentos seguintes estão informados na linha 1, quadro 3 e/ou linha 3, quadro 5: Rendimentos do trabalho assalariado:') . ' R$ ' . printValor($infoRendimentos['informeRendimento']['TOTREND']) . '.', $bordas, 'L');

// ! 7.1. Pagamentos a Planos de Saúde
if (isset($infoRendimentos['pagamentosPlanosSaude'])) {
    if (count($infoRendimentos['pagamentosPlanosSaude']) > 0) {
        $pdf->SetFont('Arial', 'B', 11);
        $pdf->Ln(4);
        $pdf->SetLeftMargin($margins['left'] + $linhaPequena + 2);
...
$pdf->SetFont('Arial', '', 10);
$bordas = 0;
$pdf->Cell(30, $altura, date('d/m/Y'), $bordas, 0, 'L');
$bordas = 'R';
