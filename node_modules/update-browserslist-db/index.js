...
function getLatestInfo(lock) {
  if (lock.mode === 'yarn') {
    if (lock.version === 1) {
      return JSON.parse(execSync('yarnpkg info caniuse-lite --json').toString()).data;
    } else {
      return JSON.parse(execSync('yarnpkg npm info caniuse-lite --json').toString());
    }
  }
  if (lock.mode === 'pnpm') {
...
function updatePackageManually(print, lock, latest) {
  ...
  let del = lock.mode === 'yarn' ? 'yarnpkg remove -W' : lock.mode + ' uninstall';
