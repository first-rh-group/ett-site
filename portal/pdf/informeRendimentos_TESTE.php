...
$stmt->execute([
    'cpf' => apenasNumeros($_GET['cpf']),
    'cnpj' => $cnpj,
]);
$idInforme = $stmt->fetch(PDO::FETCH_ASSOC)['idInforme'];
// print_r2($idInforme, __LINE__, __FILE__, __FUNCTION__);
$db = null;
$infoRendimentos = informeRendimentos_copy([
    'login' => $_GET['cpf'],
    'idInforme' => $idInforme
]);
/*
...
$infoRendimentos = informeRendimentos([
    'login' => $_SESSION['printInformeRendimentos']['login'],
    'idInforme' => $_SESSION['printInformeRendimentos']['idInforme']
]);
*/
// print_r2($infoRendimentos, __LINE__, __FILE__, __FUNCTION__);
$_SESSION['printInformeRendimentos']['sumario'] = $infoRendimentos;
...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Image('../img/brasao.png', $margins['left'], 10, 23, 0);
        ...
        $this->MultiCell(0, 6, utf8_decode('Comprovante de Rendimentos Pagos e de Imposto sobre a Renda Retido na Fonte'), $bordas, 'J');
        $this->Ln(2.5);
        ...
        $this->Line(($largura + $padding_left + 7), 3.5, $largura + $padding_left + 7, 37);
        $this->Rect($margins['left'], 3.5, $netWidth, 33.5);
        $this->Ln(2);
    }
    //Page footer
    function Footer()
...
$pdf->MultiCell(0, 4.5, utf8_decode('Verifique as condições e o prazo para a apresentação da Declaração do Imposto sobre a Renda da Pessoa Física para este ano-calendário no sítio da Secretaria da Receita Federal do Brasil na Internet, no endereço <https://www.gov.br/receitafederal/pt-br>.'), $bordas, 'J');
$pdf->Ln(1.5);
$largura = 0;
$altura = 6;
$bordas = 'LRTB';
...
$ordinate_1 = $pdf->GetY();
$bordas = 'BLR';
$pdf->MultiCell(($netWidth - $larguraValores), $altura, utf8_decode('4. Indenizações por rescisão de contrato de trabalho, inclusive a título de PDV, e por acidente de trabalho'), $bordas, 'J');
$ordinate_2 = $pdf->GetY();
$pdf->SetY($ordinate_1);
$pdf->SetX(($netWidth - $larguraValores + $margins['left']));
$bordas = 0;
...
$ordinate_1 = $pdf->GetY();
$bordas = 'BLR';
$pdf->MultiCell(($netWidth - $larguraValores), $altura, utf8_decode('5. Abono pecuniário'), $bordas, 'J');
$ordinate_2 = $pdf->GetY();
$pdf->SetY($ordinate_1);
$pdf->SetX(($netWidth - $larguraValores + $margins['left']));
$bordas = 0;
...
$ordinate_1 = $pdf->GetY();
$bordas = 'BLR';
$pdf->MultiCell(($netWidth - $larguraValores), $altura, utf8_decode('6. Juros de mora recebidos, devidos pelo atraso no pagamento de remuneração por exercício de emprego, cargo ou função'), $bordas, 'J');
$ordinate_2 = $pdf->GetY();
$pdf->SetY($ordinate_1);
$pdf->SetX(($netWidth - $larguraValores + $margins['left']));
$bordas = 0;
...
$ordinate_1 = $pdf->GetY();
$bordas = 'BLR';
$pdf->MultiCell(($netWidth - $larguraValores), $altura, utf8_decode('7. Outros rendimentos isentos'), $bordas, 'J');
$ordinate_2 = $pdf->GetY();
$pdf->SetY($ordinate_1);
$pdf->SetX(($netWidth - $larguraValores + $margins['left']));
$bordas = 0;
...
RENDIMENTOS ISENTOS ANUAIS: R$ ' . printValor($totalIsento) . '.' : '') . ($infoRendimentos['informeRendimento']['informacoes_complementares'] != '' ? '
' . $infoRendimentos['informeRendimento']['informacoes_complementares'] : '');
...
$subitem = 0;
// ! 7.X. Pagamentos a Planos de Saúde
if (isset($infoRendimentos['pagamentosPlanosSaude']) && count($infoRendimentos['pagamentosPlanosSaude']) > 0) {
    $subitem++;
    $pdf->SetFont('Arial', 'B', 11);
    $pdf->Ln(4);
    $pdf->SetLeftMargin($margins['left'] + $linhaPequena + 2);
    $pdf->Cell($tamanhoString, $altura, utf8_decode('7.' . $subitem . ' Informações Complementares (Pagamentos a Planos de Saúde)'), 0, 1, 'L');
    $bordas = 'LRTB';
    $pdf->SetX(0);
    $pdf->SetLeftMargin($margins['left']);
    $linhaAuxiliar = 38;
    // print_r2($infoRendimentos['pagamentosPlanosSaude'], __LINE__, __FILE__, __FUNCTION__);
    foreach ($infoRendimentos['pagamentosPlanosSaude'] as $key => $pagamentosOperadora) {
        $pdf->SetX(0);
        $pdf->SetLeftMargin($margins['left']);
        $pdf->SetFont('Arial', 'B', 10.5);
        $altura = 6;
        ...
        $pdf->SetFont('Arial', '', 10.5);
        $bordas = 'BLR';
        $pdf->Cell(($netWidth - $linhaAuxiliar), $altura, utf8_decode($pagamentosOperadora[0]['nomeOperadora']), $bordas, 0, 'L');
        $bordas = 'BR';
        $pdf->Cell(0, $altura, print_cnpj($pagamentosOperadora[0]['cnpjOperadora']), $bordas, 1, 'L');
        $pdf->SetFont('Arial', 'B', 10.5);
        $bordas = 'BL';
        ...
        $pdf->SetFont('Arial', '', 10.5);
        foreach ($pagamentosOperadora as $key => $pagamento) {
            if (isset($pagamento['nomeDependente'])) {
                $nomeBeneficiario = ajustaNome($pagamento['nomeDependente']) . ' (dependente)';
                $cpf = print_cpf($pagamento['cpfDependente']);
            } else {
                $nomeBeneficiario = ajustaNome($infoRendimentos['informeRendimento']['NOMEFUNCIONARIO']);
                $cpf = print_cpf($infoRendimentos['informeRendimento']['CPF']);
            }
            $bordas = 'BL';
            $pdf->Cell(($netWidth - ($linhaAuxiliar - 10) - $linhaAuxiliar), $altura, utf8_decode($nomeBeneficiario), $bordas, 0, 'L');
            $bordas = 'BL';
            $pdf->Cell(($linhaAuxiliar - 10), $altura, $cpf, $bordas, 0, 'L');
            $bordas = 'BLR';
            $pdf->Cell(0, $altura, 'R$ ' . printValor($pagamento['VALOR']), $bordas, 1, 'L');
        }
        $pdf->Ln(3);
    }
}
...
if ($infoRendimentos['informeRendimento']['PENSAO'] > 0) {
    ...
    $pdf->SetFont('Arial', 'B', 11);
    $pdf->SetLeftMargin($margins['left'] + $linhaPequena + 2);
    $pdf->Cell($tamanhoString, $altura, utf8_decode('7.' . $subitem . ' Desconto de Pensão Alimentícia'), 0, 1, 'L');
    $bordas = 'LRTB';
    $pdf->SetX(0);
    $pdf->SetLeftMargin($margins['left']);
    $linhaAuxiliar = 38;
    ...
    $altura = 6;
    $bordas = 'TLR';
    $pdf->Cell(($netWidth - $linhaAuxiliar), $altura, '', $bordas, 0, 'L');
    $bordas = 'TR';
    $pdf->Cell(0, $altura, 'Valor', $bordas, 1, 'L');
    $pdf->SetFont('Arial', '', 10.5);
    $bordas = 'BLR';
    // print_r2($beneficiario, __LINE__, __FILE__, __FUNCTION__);
    foreach ($infoRendimentos['informeRendimento']['beneficiarios'] as $key => $beneficiario) {
        $pdf->Cell(($netWidth - $linhaAuxiliar), $altura, $beneficiario['NOME'] . ', nasc.: ' . $beneficiario['NASCIMENTO'] . ', CPF ' . $beneficiario['CPF'], $bordas, 0, 'L');
        $bordas = 'BR';
        $pdf->Cell(0, $altura, 'R$ ' . $beneficiario['VALOR'], $bordas, 1, 'L');
        $bordas = 'BLR';
    }
    $bordas = 'BL';
    $pdf->Ln(3);
